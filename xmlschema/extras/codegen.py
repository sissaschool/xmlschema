...
def xsd_qname(name):
    return '{{{}}}{}'.format(XSD_NAMESPACE, name)
...
class AbstractGenerator(ABC, metaclass=GeneratorMeta):
    ...
    def __repr__(self):
        if self.schema.url:
            return '{}(schema={!r})'.format(self.__class__.__name__, self.schema.name)
        return '{}(namespace={!r})'.format(self.__class__.__name__, self.schema.target_namespace)
