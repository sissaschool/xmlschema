...
from sdcvalidator.exceptions import XMLSchemaAttributeError, XMLSchemaTypeError, \
    XMLSchemaValueError
from sdcvalidator.aliases import ElementType, XMLSourceType, NsmapType, BaseXsdType, \
    DecodeType, EncodeType
from sdcvalidator.converters import ElementData, XMLSchemaConverter
from sdcvalidator.resources import XMLResource
from sdcvalidator.utils.qnames import get_namespace, get_prefixed_qname, \
    local_name, update_namespaces, get_namespace_map
from sdcvalidator.utils.decoding import raw_encode_value
from sdcvalidator import core

if TYPE_CHECKING:
    from .core import XMLSchemaValidationError, XsdElement
...
class DataElement(MutableSequence['DataElement']):
    ...
    def __setattr__(self, key: str, value: Any) -> None:
        if key == 'xsd_element':
            if not isinstance(value, core.XsdElement):
            ...
            if not isinstance(value, (core.XsdSimpleType, core.XsdComplexType)):
    ...
    def iter_errors(self, use_defaults: bool = True,
        ...
        for result in self._encoder.iter_encode(self, **kwargs):
            if isinstance(result, core.XMLSchemaValidationError):
                yield result
            else:
                del result
    ...
    def encode(self, validation: str = 'strict', **kwargs: Any) -> EncodeType[ElementType]:
        ...
        if self._encoder is not None:
            ...
            encoder = core.XMLSchema.builtin_types()['anyType']
        else:
            raise XMLSchemaValueError("%r has no schema bindings" % self)
...
class DataBindingMeta(ABCMeta):
    ...
    def __new__(mcs, name: str, bases: tuple[Type[Any], ...],
        ...
        if not isinstance(xsd_element, core.XsdElement):
